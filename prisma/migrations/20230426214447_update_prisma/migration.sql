/*
  ...
  - Changed the type of `A` on the `_FavoriteToList` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.

*/
-- DropForeignKey
ALTER TABLE "_FavoriteToList" DROP CONSTRAINT "_FavoriteToList_A_fkey";
...
-- AlterTable
ALTER TABLE "_FavoriteToList" DROP COLUMN "A",
